...
- Uses startup script to pull llama3.2:latest model on first run

**Startup Script**: `ollama-startup.sh`
- Starts Ollama server
- Waits for server to be ready
- Pulls llama3.2:latest model (if not already present)
...
- Healthcheck confirms llama3.2:latest model is available before marking container healthy
...
**To Avoid:**
- Do NOT make model configurable - always use llama3.2:latest
...
3. **ollama**: Ollama AI server
   ...
   - Health check: `curl -s http://localhost:11434/api/tags | grep -q llama3.2:latest`
   - Volume: `ollama-data` (persists downloaded models)
   - Custom entrypoint: `ollama-startup.sh` (pulls llama3.2:latest model on first run)
